...
//---------------------------------------------------------------------------------------------------------------------------
// проверка на стрит-флеш
bool HandStrength::match_straitflash(const unique_ptr<vector<Card> > & combo_ptr) const
{
    if (combo_ptr->size() < 5)
        return false;
    else
...
bool HandStrength::match_kare(const unique_ptr<vector<Card> > & combo_ptr) const
{
    ...
    for (unsigned count = 0; (combo_ptr->size() - count) >= 4; ++count)
    {
        c_true = 0;
...
bool HandStrength::match_fullhouse(const unique_ptr<vector<Card> > & combo_ptr) const
{
    ...
    for (unsigned count = 0; (temp_arr_ptr->size() - count) >= 5; ++count)
    {
...
//---------------------------------------------------------------------------------------------------------------------------
// проверка на флеш
bool HandStrength::match_flush(const unique_ptr<vector<Card> > & combo_ptr) const
{
    if (combo_ptr->size() < 5)
        return false;
    else
...
//---------------------------------------------------------------------------------------------------------------------------
// проверка на стрит
bool HandStrength::match_strait(const unique_ptr<vector<Card> > & combo_ptr) const
{
    if (combo_ptr->size() < 5)
        return false;
    else {
