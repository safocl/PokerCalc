...
//---------------------------------------------------------------------------------------------------------------------------
void parallel_genOneBoardCard(vector<Card> && board, Deck && deck, Hand && hero_h, Hand && opp_h, 
             ...
             std::atomic<unsigned long long> & fullhouse, std::atomic<unsigned long long> & kare, std::atomic<unsigned long long> & straitflash)
{
    genOneBoardCard(board, deck, hero_h, opp_h, [&](){
        genOneBoardCard(board, deck, hero_h, opp_h, [&](){
            ...
            thread thread1(parallel_genOneBoardCard, board, move(deck), hero_h, hero_h, min_pos1, max_pos1,
            ...
            thread thread2(parallel_genOneBoardCard, board, move(deck), hero_h, hero_h, min_pos2, max_pos2,
                           ref(hight), ref(pair), ref(twopair),ref(set),ref(strait),
                           ref(flash),ref(fullhouse),ref(kare),ref(straitflash));
            auto min_pos3 = max_pos2;
            auto max_pos3 = max_pos1 * 3;
            thread thread3(parallel_genOneBoardCard, board, move(deck), hero_h, hero_h, min_pos3, max_pos3,
                           ref(hight), ref(pair), ref(twopair),ref(set),ref(strait),
                           ref(flash),ref(fullhouse),ref(kare),ref(straitflash));
            auto min_pos4 = max_pos3;
            auto max_pos4 = deck.size();
            thread thread4(parallel_genOneBoardCard, board, move(deck), hero_h, hero_h, min_pos4, max_pos4,
                           ref(hight), ref(pair), ref(twopair),ref(set),ref(strait),
                           ref(flash),ref(fullhouse),ref(kare),ref(straitflash));
